...
import io, os, sys, getopt, logging, shutil, tempfile
                ...
                while True:
                    ...
                    if action[0] in ('x', 'X', 'q', 'Q'):
                        ...
                        try:
                            ...
                            new_word = new_word.decode(Config['TERMINAL_ENCODING'])
                        except KeyboardInterrupt:
...
def main(argv=None):
    ...
    if enter_check:
        ...
        os.close(fd)
        with io.open(tmpfile, 'w', encoding=Config['ENCODING']) as tmpobj:
            with io.open(filename, encoding=Config['ENCODING']) as fileobj:
